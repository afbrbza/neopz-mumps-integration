/**
 * @file
 * @brief Contains the implementation of the TPZSpStructMatrixSpooles methods. 
 */
...
TPZSpStructMatrixSpooles<TVar, TPar> * TPZSpStructMatrixSpooles<TVar,TPar>::Clone(){
    return new TPZSpStructMatrixSpooles(*this);
}

template <class TVar, class TPar>
TPZMatrix<TVar> *TPZSpStructMatrixSpooles<TVar, TPar>::Create() {
...
template<class TVar, class TPar>
TPZMatrix<TVar> *
TPZSpStructMatrixSpooles<TVar,TPar>::SetupMatrixData(TPZStack<int64_t> & elgraph,
                                              TPZVec<int64_t> &elgraphindex){
    
    const int64_t neq = this->fEquationFilter.NActiveEquations();
    TPZFYsmpMatrixSpooles<TVar> * mat = new TPZFYsmpMatrixSpooles<TVar>(neq,neq);
...
template class TPZSpStructMatrixSpooles<STATE,TPZStructMatrixOR<STATE>>;
template class TPZSpStructMatrixSpooles<STATE,TPZStructMatrixOT<STATE>>;
template class TPZSpStructMatrixSpooles<STATE,TPZStructMatrixTBBFlow<STATE>>;
template class TPZSpStructMatrixSpooles<STATE,TPZStructMatrixOMPorTBB<STATE>>;
